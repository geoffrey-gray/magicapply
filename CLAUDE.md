...
- **Router regex tables live in YAML** (`src/magicapply/infrastructure/browser/ats/resources/router_rules.yaml` as the package default; operator override at `<config_root>/router_rules.yaml`) with a `RouterRules` Pydantic model. Loaded via `RouterRules.load_default()` (lru-cached, `importlib.resources`) and injected into `AnswerRouter` in `composition.build_application_data`. Sections: `identity`, `yes_no`, `dei`, `consent`, `optional_checkbox`, `sms_opt_in`, `handler_owned_widget` (label-based skip), `handler_owned_variants` (variant-based skip), plus `experience_years` / `skill_screening` single-regexes. Adding a new employer quirk is a YAML edit, no code change.
- **Rotating proxy pool** (`infrastructure/browser/proxy_pool.py`) — `ProxyPool` + `ProxyProvider` Protocol + `StaticListProvider` / `FreeListScraperProvider` / `FallbackProvider`. Config-driven via `configs/base_config.yaml::proxies`. Cloudflare-adjacent adapters (Indeed / Glassdoor) burn dead proxies and requeue blocked queries; the `PlaywrightSession(proxy=…)` path spins per-proxy `BrowserContext`s with UA + viewport rotation. Default `enabled: false` → adapters fall back to direct fetches (pre-Phase-A behavior).
- **Apply throttle** (`domain/apply/throttle.py`) — `ApplyThrottle` with per-ATS hourly + daily caps and a global cap. Fires in `ApplyPipeline.apply_one` just before `handler.apply(...)`. On breach: application stays TAILORED so the next batch picks it up when the window rolls. Config lives at `base_config.yaml::apply_throttle`.
- **Fair cross-source dedup** — `dedupe_by_key(jobs, *, source_scorer=…)` in `domain/jobs/dedup.py`. Default (no scorer) preserves first-wins. `DiscoveryPipeline` wires a "recent apply count by source" scorer so cross-source collisions distribute future apply traffic across LinkedIn / Indeed / Glassdoor / greenhouse-boards.
- **Session-cookie auth for Cloudflare-blocked sources** — `parse_cookie_string(cookie_string, *, domain)` in `sources/base.py` parses browser `Cookie:` header strings into Playwright cookie dicts. `INDEED_SESSION_COOKIES` + `GLASSDOOR_SESSION_COOKIES` (legacy `GLASSDOOR_SESSION` still supported) inject via `session.add_cookies()` at discover-time, mirroring `LINKEDIN_LI_AT`. When cookies are present, the adapter's `_effective_proxy_pool()` returns None — auth sessions and per-proxy contexts don't mix. Config surface stays in `.env` (secrets, not YAML). See runbook §13.
